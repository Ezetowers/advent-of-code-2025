...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    loop {
        ...
        if rolls_removed == 0 {
            debug!("No rolls removed in last iteration. Quitting");
            break;
        }

        total += rolls_removed;
        // NOTE: This is done just to show the output of every loop with the rolls removed
        // in that iteration. The exercise should work without this step
        for i in 0..grid_marked.len() {
            grid_marked[i].iter_mut().for_each(|elem| {
                if *elem == 'x' {
                    *elem = '.';
                }
            });
        ...
        grid = grid_marked;
        debug!("Rolls Removed: {}", rolls_removed);
        debug!("");
        rolls_removed = 0;
    }
